# Context Pack — medium (the four decision-style golds)

The four hype golds — **market-desk, browser-agent, agent-economy, voice-agent** —
each taught scene-by-scene: how real Sim blocks compose into a whole video with taste.
This is the *decision-style* corpus — composition, run economy, the EXCITE register,
and exit-state continuity (one run carried across freeze-cuts).

Read it WHOLE (page in ≤1000-line windows — the Read tool caps at ~25k tokens, ≈1000
dense lines). Imitate the decision style, never the literal content.

**When to use high instead:** if the video's POINT is a specific *mechanic* — a loop, a
subworkflow, custom tools, branching — use `context-pack-high.md`. It prepends the
academy CORE PATTERNS that teach those mechanics precisely (the golds are all fan/parallel-
shaped, so golds-only can mis-draw e.g. a sequential loop as a Parallel fan).
